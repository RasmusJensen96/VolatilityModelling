GJRGARCH11_Filter <- function(vY, dOmega, dAlpha, dBeta, dAlphaplus) {
  ...
  for (t in 2:iT) {
    vSigma2[t] = dOmega + dAlpha * vY[t - 1] ^ 2 + dBeta * vSigma2[t - 1] + dAlphaplus * vYplus[t-1]^2
...
LLH_GJRGARCH11 <- function(vPar, vY) {
  ...
  dBeta  = vPar[3]
  dAlphaplus = vPar[4]
  dLLK = GJRGARCH11_Filter(vY, dOmega, dAlpha, dBeta, dAlphaplus)$dLLK
  return(-dLLK)
}
StationarityConstrGJR <- function(vPar, ...) {
  ...
  dBeta  = vPar[3]
  dAlphaplus = vPar[4]
  return(dAlpha + dBeta + dAlphaplus)
}
